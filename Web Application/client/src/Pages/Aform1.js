...
function Aform1() {
    ...
    const getForm1 = () => {
        Axios.post("http://localhost:3001/res1").then((response) => {
    ...
    const clearForm1 = () => {
        Axios.post("http://localhost:3001/clean1").then((response) =>{
            console.log("Cleaned!");
    ...
    return (
        <div>
            <h1>Hostel Leave Form Responses</h1>
            <button onClick={getForm1}>Get Responses</button>
            <br />
            <br />
            <div align="center">
            <div style={{display: "None", margin: "10px", fontSize: "16px"}} id="disp"></div>  
            <br />
            <button style={{display: "None"} } id="disp1" onClick={clearForm1}>Submit Responses</button>          
            </div>
        </div>
    );
}
export default Aform1;